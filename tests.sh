...
# Backwards compat: a leading integer means "long with this NCPU"
if [[ "$LEVEL" =~ ^[0-9]+$ ]]; then
  export NCPU="$LEVEL"
  LEVEL="long"
fi

# Auto-activate the dante_tir conda env if available and we aren't in
# any conda env yet. Harmless on systems without conda.
if command -v conda >/dev/null 2>&1 && [ -z "${CONDA_DEFAULT_ENV:-}" ]; then
  eval "$(conda shell.bash hook)"
  if conda env list | grep -q '^dante_tir '; then
